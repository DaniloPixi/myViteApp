<template>
    <div class="daily-quest-card">
      ...
      <div class="dq-footer">
        <button
          ...
          @click="completeQuest"
        >
          <span v-if="quest?.completed">Quest completed ✨</span>
 ...
 async function completeQuest() {
  if (quest.value?.completed) return;
  loading.value = true;
  try {
    // 1) Update Firestore via your composable
    ...
    // 2) Tell your backend to send a push notification
    const payload = {
    ...
    try {
      const res = await fetch('/.netlify/functions/api/quests/notify-completed', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          // ⚠️ If your other authenticated calls add an auth token,
          // copy that logic here as well.
      ...
      if (!res.ok) {
        console.warn('[DailyQuestWidget] notify-completed failed:', await res.text());
      }
    } catch (notifyError) {
      console.warn('[DailyQuestWidget] Failed to call notify-completed:', notifyError);
    }
  } catch (e) {
