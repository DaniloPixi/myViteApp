...
.modal-content { background: #1e1e1e; padding: 2rem; border-radius: 12px; border: 1px solid #444; box-shadow: 0 5px 15px rgba(0,0,0,0.5); max-width: 500px; width: 90%; }
h3 { color: #42b883; text-align: center; margin-top: 0; margin-bottom: 2rem; }

/* Form styles */
.plan-form { display: flex; flex-direction: column; gap: 1rem; }
.form-group { width: 100%; }
.form-group label { display: block; margin-bottom: 0.5rem; color: #ccc; }
input { box-sizing: border-box; width: 100%; padding: 0.8em 1em; border-radius: 8px; border: 1px solid #444; background-color: #1a1a1a; color: #fff; font-size: 1em; }

/* Time & Duration Group */
.time-duration-group { display: flex; align-items: stretch; gap: 1rem; }
.time-duration-group input[type="time"] { flex-basis: 150px; flex-grow: 0; }
.duration-buttons { display: flex; flex-grow: 1; justify-content: center; gap: 0.5rem; }
...
.modal-actions button { padding: 0.7em 1.4em; border-radius: 30px; border: none; font-size: 1em; font-weight: 600; cursor: pointer; }
.confirm-button { background-color: #42b883; color: white; }
.cancel-button { background-color: #555; color: white; }
.error-message { color: #ff6b6b; text-align: center; margin-top: 1rem; }
